...
void CAutoAirblast::Run(CBaseEntity* g_pLocalPlayer, CUserCmd* g_pUserCmd)
{
	...
	if (!gCvars.triggerbot_autoairblast)
		return;
	if (!Util->IsKeyPressed(gCvars.triggerbot_autoairblast_key))
		return;
	...
	for (int i = 0; i < gInts.EntList->GetHighestEntityIndex(); i++)
	{
		...
		//	gInts.Engine->GetNetChannelInfo()->GetLatency(FLOW_OUTGOING);
	...
	if (!gCvars.triggerbot_autoairblast_legit)
	{
		AimAt(g_pLocalPlayer->GetEyePosition(), closest_vec, g_pUserCmd, gCvars.triggerbot_autoairblast_silent);
	}

	// Airblast
	g_pUserCmd->buttons |= IN_ATTACK2;

	if (!gCvars.triggerbot_autoairblast_silent)
