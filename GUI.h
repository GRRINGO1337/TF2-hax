...
class CLabel : public CObject
{
	...
	bool m_centered;
public:
	CLabel(std::wstring text = L"", HFont font = 0, Color color = Color(0), bool centered = false);
	void Paint(int t_x = 0, int t_y = 0, float* controlheight = nullptr);
};
...
class CCheckBox2 : public CObject
{
	std::wstring m_label;
	bool* m_val;
public:
	CCheckBox2(std::wstring label = L"", bool* value = nullptr);
	void Paint(int t_x = 0, int t_y = 0, float* controlheight = nullptr);
	void Click();
};
